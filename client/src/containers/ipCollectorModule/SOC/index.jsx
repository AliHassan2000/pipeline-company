...
const Index = (props) => {
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      ...
      try {
        await axios
          .get(baseUrl + "/getAllSocIpCollectorDates")
          .then((response) => {
        ...
        const status = await axios.get(
          baseUrl + "/getSocIpCollectorScriptStatus"
        );
        console.log("popo");
        console.log(status);
        ...
        const res = await axios.get(baseUrl + "/getAllSocIpCollector");
        excelData = res.data;
  ...
  const jsonToExcel = (seedData) => {
    ...
    XLSX.utils.book_append_sheet(wb, binarySeedData, "socipcollector");
    XLSX.writeFile(wb, "socipcollector.xlsx");
  ...
  const fetch = async () => {
    ...
    setBackgroundColor("red");
    await axios
      .get(baseUrl + "/fetchSocIpCollector")
      .then((response) => {
  ...
  const handleSelectChange = async (value) => {
    setLoading(true);
    await axios
      .post(baseUrl + "/getAllSocIpCollectorByDate", { date: value })
      .then((response) => {
        excelData = response.data;
  ...
  const exportFiltered = async () => {
    setExportLoading(true);
    jsonToExcel(dataSource, "filtersocipCollector.xlsx");
    setExportLoading(false);
  };

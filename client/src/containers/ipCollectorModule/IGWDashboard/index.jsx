...
const Index = (props) => {
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      setLoading(true);
      try {
        const res = await axios.get(baseUrl + "/igwIpamIpCount");
        setInventoryCounts(res.data);

        const tags = await axios.get(baseUrl + "/igwIpamPerRegion");
        setTags(tags.data);

        const res5 = await axios.get(baseUrl + "/igwIpamSites");
  ...
  return (
    ...
    >
      <StyledHeading>IGW Dashboard</StyledHeading>
      <Row>
        <Col span={24} style={{}}>
          {inventoryCounts ? (

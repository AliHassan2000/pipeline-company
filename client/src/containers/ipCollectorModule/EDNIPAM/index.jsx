...
const Index = (props) => {
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      ...
      try {
        await axios.get(baseUrl + "/getAllEdnIpamDates").then((response) => {
          setDates(response.data);
          console.log(response);
        });

        const status = await axios.get(baseUrl + "/getEdnIpamScriptStatus");
        console.log("popo");
        console.log(status);
        ...
        const res = await axios.get(baseUrl + "/getAllEdnIpam");
        excelData = res.data;
  ...
  const jsonToExcel = (seedData) => {
    ...
    XLSX.utils.book_append_sheet(wb, binarySeedData, "ednipcollector");
    XLSX.writeFile(wb, "ednipcollector.xlsx");
  ...
  const handleSelectChange = async (value) => {
    setLoading(true);
    await axios
      .post(baseUrl + "/getAllEdnIpamByDate", { date: value })
      .then((response) => {
        excelData = response.data;
  ...
  const exportFiltered = async () => {
    setExportLoading(true);
    jsonToExcel(dataSource, "filteredednipcollector.xlsx");
    setExportLoading(false);
  };

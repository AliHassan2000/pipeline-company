...
import { Table, Button, Space, notification, Spin, Input, Select } from "antd";
...
const Index = (props) => {
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      ...
      try {
        await axios
          .get(baseUrl + "/getAllEdnPhysicalMappingFailedDevicesDates")
          .then((response) => {
        ...
        const res = await axios.get(
          baseUrl + "/getAllEdnPhysicalMappingFailedDevices"
        );
        excelData = res.data;
  ...
  const jsonToExcel = (seedData) => {
    ...
    XLSX.utils.book_append_sheet(
      wb,
      binarySeedData,
      "physicalmappingednfaileddevices"
    );
    XLSX.writeFile(wb, "physicalmappingednfaileddevices.xlsx");
  ...
  const handleSelectChange = async (value) => {
    setLoading(true);
    await axios
      .post(baseUrl + "/getEdnPhysicalMappingFailedDevicesByDate", {
        date: value,
      })
      .then((response) => {
  ...
  return (
    <>
      <Spin tip="Loading..." spinning={loading}>
        ...
        <StyledHeading>
          Physical Mapping EDN Failed Devices
          <span
            style={{
              float: "right",
              fontSize: "14px",

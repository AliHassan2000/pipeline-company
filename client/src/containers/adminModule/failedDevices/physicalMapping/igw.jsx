...
const Index = (props) => {
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      ...
      try {
        await axios
          .get(baseUrl + "/getAllIgwPhysicalMappingFailedDevicesDates")
          .then((response) => {
        ...
        const res = await axios.get(
          baseUrl + "/getAllIgwPhysicalMappingFailedDevices"
        );
        excelData = res.data;
  ...
  const jsonToExcel = (seedData) => {
    ...
    XLSX.utils.book_append_sheet(
      wb,
      binarySeedData,
      "physicalmappingigwfaileddevices"
    );
    XLSX.writeFile(wb, "physicalmappingigwfaileddevices.xlsx");
  ...
  const handleSelectChange = async (value) => {
    setLoading(true);
    await axios
      .post(baseUrl + "/getIgwPhysicalMappingFailedDevicesByDate", {
        date: value,
      })
      .then((response) => {
  ...
  return (
    <>
      <Spin tip="Loading..." spinning={loading}>
        ...
        <StyledHeading>
          Physical Mapping IGW Failed Devices
          <span
            style={{
              float: "right",
              fontSize: "14px",

...
const Index = (props) => {
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      ...
      try {
        await axios
          .get(baseUrl + "/getAllF5FailedDevicesDates")
          .then((response) => {
            setDates(response.data);
            console.log(response);
          });

        const res = await axios.get(baseUrl + "/getAllF5FailedDevices");
        excelData = res.data;
  ...
  const jsonToExcel = (seedData) => {
    ...
    XLSX.utils.book_append_sheet(wb, binarySeedData, "f5faileddevices");
    XLSX.writeFile(wb, "f5faileddevices.xlsx");
  ...
  const handleSelectChange = async (value) => {
    setLoading(true);
    await axios
      .post(baseUrl + "/getF5FailedDevicesByDate", { date: value })
      .then((response) => {

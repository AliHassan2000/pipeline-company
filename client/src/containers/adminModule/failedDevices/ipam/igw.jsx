...
const Index = (props) => {
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      ...
      try {
        await axios
          .get(baseUrl + "/getAllIgwIpamFailedDevicesDates")
          .then((response) => {
            setDates(response.data);
            console.log(response);
          });

        const res = await axios.get(baseUrl + "/getAllIgwIpamFailedDevices");
        excelData = res.data;
  ...
  const jsonToExcel = (seedData) => {
    ...
    XLSX.utils.book_append_sheet(wb, binarySeedData, "ipamigwfaileddevices");
    XLSX.writeFile(wb, "ipamigwfaileddevices.xlsx");
  ...
  const handleSelectChange = async (value) => {
    setLoading(true);
    await axios
      .post(baseUrl + "/getIgwIpamFailedDevicesByDate", { date: value })
      .then((response) => {
  ...
  return (
    <>
      <Spin tip="Loading..." spinning={loading}>
        ...
        <StyledHeading>
          IP Collector IGW Failed Devices
          <span
            style={{
              float: "right",
              fontSize: "14px",

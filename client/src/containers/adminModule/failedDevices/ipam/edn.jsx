...
const Index = (props) => {
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      ...
      try {
        await axios
          .get(baseUrl + "/getAllEdnIpamFailedDevicesDates")
          .then((response) => {
            setDates(response.data);
            console.log(response);
          });

        const res = await axios.get(baseUrl + "/getAllEdnIpamFailedDevices");
        excelData = res.data;
  ...
  const jsonToExcel = (seedData) => {
    ...
    XLSX.utils.book_append_sheet(wb, binarySeedData, "ipamednfaileddevices");
    XLSX.writeFile(wb, "ipamednfaileddevices.xlsx");
  ...
  const handleSelectChange = async (value) => {
    setLoading(true);
    await axios
      .post(baseUrl + "/getEdnIpamFailedDevicesByDate", { date: value })
      .then((response) => {
  ...
  return (
    <>
      <Spin tip="Loading..." spinning={loading}>
        ...
        <StyledHeading>
          IP Collector EDN Failed Devices
          <span
            style={{
              float: "right",
              fontSize: "14px",

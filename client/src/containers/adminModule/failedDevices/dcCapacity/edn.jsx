...
const Index = (props) => {
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      ...
      try {
        await axios
          .get(baseUrl + "/getAllEdnDcCapacityFailedDevicesDates")
          .then((response) => {
            setDates(response.data);
            console.log(response);
          });

        const res = await axios.get(baseUrl + "/getAllDcCapacityFailedDevices");
        excelData = res.data;
  ...
  const jsonToExcel = (seedData) => {
    ...
    XLSX.utils.book_append_sheet(
      wb,
      binarySeedData,
      "dccapacityednfaileddevices"
    );
    XLSX.writeFile(wb, "dccapacityednfaileddevices.xlsx");
  ...
  const handleSelectChange = async (value) => {
    setLoading(true);
    await axios
      .post(baseUrl + "/getEdnDcCapacityFailedDevicesByDate", { date: value })
      .then((response) => {
  ...
  return (
    <>
      <Spin tip="Loading..." spinning={loading}>
        ...
        <StyledHeading>
          DC Capacity EDN Failed Devices
          <span
            style={{
              float: "right",
              fontSize: "14px",

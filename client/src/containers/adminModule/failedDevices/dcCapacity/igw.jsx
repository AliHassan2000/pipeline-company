...
const Index = (props) => {
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      ...
      try {
        await axios
          .get(baseUrl + "/getAllIgwDcCapacityFailedDevicesDates")
          .then((response) => {
        ...
        const res = await axios.get(
          baseUrl + "/getAllIgwDcCapacityFailedDevices"
        );
        excelData = res.data;
  ...
  const jsonToExcel = (seedData) => {
    ...
    XLSX.utils.book_append_sheet(
      wb,
      binarySeedData,
      "ipamigwdccapacityfaileddevices"
    );
    XLSX.writeFile(wb, "ipamigwdccapacityfaileddevices.xlsx");
  ...
  const handleSelectChange = async (value) => {
    setLoading(true);
    await axios
      .post(baseUrl + "/getIgwDcCapacityFailedDevicesByDate", { date: value })
      .then((response) => {
  ...
  return (
    <>
      <Spin tip="Loading..." spinning={loading}>
        ...
        <StyledHeading>
          DC Capacity IGW Failed Devices
          <span
            style={{
              float: "right",
              fontSize: "14px",

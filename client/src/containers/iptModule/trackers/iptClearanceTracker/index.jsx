...
const Index = (props) => {
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      ...
      setLoading(true);
      try {
        const res = await axios.get(baseUrl + "/ipClearanceTracker");
        excelData = res.data;
  ...
  const postNe = async (seed) => {
    setLoading(true);
    await axios
      .post(baseUrl + "/ipClearanceTracker", seed)
      .then((response) => {
  ...
  const columns = [
    ...
    {
      dataIndex: "ipt_team_assignee",
      key: "ipt_team_assignee",
      // ...getColumnSearchProps("switch_name"),
      ...getColumnSearchProps(
        "ipt_team_assignee",
        "IPT Team Assignee",
  ...
  const handleDelete = () => {
    if (selectedRowKeys.length > 0) {
      axios
        .delete(baseUrl + "/ipClearanceTracker", {
          data: {
            ips: selectedRowKeys,
          },
        })
  ...
  const jsonToExcel = (seedData) => {
    ...
    XLSX.utils.book_append_sheet(wb, binarySeedData, "iptclearancetracker");
    XLSX.writeFile(wb, "iptclearancetracker.xlsx");
    ...
    <>
      <Spin tip="Loading..." spinning={loading}>
        ...
        <Table
          ...
          size="small"
          scroll={{ x: 3700, y: height - 350 }}
          onChange={handleChange}
          rowSelection={rowSelection}
          columns={columns}
          dataSource={dataSource}
          rowKey="ip_clearance_tracker_id"
        />
      </Spin>
    </>
  );

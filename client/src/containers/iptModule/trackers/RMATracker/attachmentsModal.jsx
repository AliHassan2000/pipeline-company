...
const Index = ({ record, setIsAttachmentModalVisible }) => {
    ...
    const serviceCalls = async () => {
      ...
      try {
        const res = await axios.get(
          baseUrl +
            `/getAttachmentsById?trackerId=${record.ipt_rma_tracker_id}&category=ipt_rma_tracker`
        );
        excelData = res.data;
  ...
  const addAttachments = async (device) => {
    let url = "/iptRMATracker?category=ipt_rma_tracker";
    try {
      await axios
  ...
  const handleSubmit = () => {
    if (attachments.length == 0) {
      ...
      let data = [
        {
          ipt_rma_tracker_id: record.ipt_rma_tracker_id,
          rma_order_number: record.rma_order_number,
          service_request_number: record.service_request_number,
          serial_number: record.serial_number,
          mac: record.mac,
          user_info_and_device_impacted_details:
            record.user_info_and_device_impacted_details,
          rma_ordered_date: record.rma_ordered_date,
          fe_receiving_the_rma_part_from_dhl:
            record.fe_receiving_the_rma_part_from_dhl,
          current_status: record.current_status,
          actual_rma_received_date: record.actual_rma_received_date,
          engineer_handling_the_rma: record.engineer_handling_the_rma,
          pickup_date_scheduled_in_airway_bill:
            record.pickup_date_scheduled_in_airway_bill,
          fe_delivering_the_device_to_dhl:
            record.fe_delivering_the_device_to_dhl,

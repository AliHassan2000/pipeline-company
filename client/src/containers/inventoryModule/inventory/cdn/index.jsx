...
const Index = (props) => {
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      ...
      try {
        const res = await axios.get(baseUrl + "/getAllCDN");
        excelData = res.data;
  ...
  const postNe = async (seed) => {
    setLoading(true);
    await axios
      .post(baseUrl + "/addCDNDevices", seed)
      .then((response) => {
  ...
  const handleDelete = () => {
    if (selectedRowKeys.length > 0) {
      axios
        .post(baseUrl + "/deleteCDNDevice", {
          technology: selectedRowKeys,
        })
        .then(() => {
          const promises = [];
          promises.push(
  ...
  const jsonToExcel = (seedData) => {
    ...
    XLSX.utils.book_append_sheet(wb, binarySeedData, "cdn");
    XLSX.writeFile(wb, "cdn.xlsx");

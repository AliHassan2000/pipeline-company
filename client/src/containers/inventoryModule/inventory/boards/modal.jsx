...
const AddDeviceModal = (props) => {
  ...
  /////////////////////////////
  let [ip, setIp] = useState(device ? getString(device.board_id) : "");
  ...
  const postDevice = async (device) => {
    try {
      //console.log(device);
      await axios
        .post(baseUrl + "/editBoards", device)
        .then(() => {
          const promises = [];
          promises.push(
            axios
      ...
      <form onSubmit={handleSubmit}>
        <Row gutter={30}>
          <Col span={24} style={{ textAlign: "center" }}>
            <p style={{ fontSize: "22px" }}>{device ? "Edit" : "Add"} Board</p>
          </Col>
          <Col span={24}>
            <InputWrapper>
              Board Id: &nbsp;<span style={{ color: "red" }}>*</span>
              &nbsp;&nbsp;
              {device ? (

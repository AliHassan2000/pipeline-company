...
const Index = (props) => {
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      ...
      try {
        const res = await axios.get(baseUrl + "/getAllDomain");
        excelData = res.data;
  ...
  const handleDelete = () => {
    if (selectedRowKeys.length > 0) {
      axios
        .post(baseUrl + "/deleteDomain", {
          domain_id: selectedRowKeys,
        })
        .then(() => {
          const promises = [];
          promises.push(
  ...
  const jsonToExcel = (seedData) => {
    ...
    XLSX.utils.book_append_sheet(wb, binarySeedData, "domain");

...
const AddDeviceModal = (props) => {
  ...
  let [tfun, setTFun] = useState(device ? getString(device.tfun) : "");
  ...
  const postDevice = async (device) => {
    try {
      await axios
        .post(baseUrl + "/addFunction", device)
        .then(() => {
          const promises = [];
          promises.push(
            axios
        ...
        <Row gutter={30}>
          ...
          <Col span={24}>
            ...
            <InputWrapper>
              ...
              &nbsp;&nbsp;
              {device ? (
                <StyledInput value={tfun} readonly />
              ) : (
                <StyledInput
                  value={tfun}
                  onChange={(e) => setTFun(e.target.value)}
                  required
                />
              )}
            </InputWrapper>

...
const AddDeviceModal = (props) => {
  ...
  /////////////////////////////
  let [ip, setIp] = useState(device ? getString(device.subboard_id) : "");
  ...
  const postDevice = async (device) => {
    try {
      //console.log(device);
      await axios
        .post(baseUrl + "/editSubBoards", device)
        .then(() => {
          const promises = [];
          promises.push(
            axios
        ...
        <Row gutter={30}>
          <Col span={24} style={{ textAlign: "center" }}>
            <p style={{ fontSize: "22px" }}>
              {device ? "Edit" : "Add"} SubBoard
            </p>
          </Col>
          <Col span={24}>
            <InputWrapper>
              SubBoard Id: &nbsp;<span style={{ color: "red" }}>*</span>
              &nbsp;&nbsp;
              {device ? (

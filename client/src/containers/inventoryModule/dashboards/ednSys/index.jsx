...
const Index = (props) => {
  ...
  const [tagIdEdnSysVDate, setTagIdEdnSysVDate] = useState(null);
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      setLoading(true);
      try {
        const tags = await axios.get(baseUrl + "/tagIdEdnSys");
        setTags(tags.data);
        console.log(tags.data);

        const eos = await axios.get(baseUrl + "/eosExpiryEdnSys");
        setEOSExpiry(eos.data);
        console.log(eos.data);

        const res = await axios.get(baseUrl + "/ednSysInventoryCounts ");
        ...
        const res2 = await axios.get(baseUrl + "/ednSysVirtual ");
        setVirtualTypes(res2.data);
        // console.log(res2.data);

        const res3 = await axios.get(baseUrl + "/tagIdEdnSysVDate ");
        setTagIdEdnSysVDate(res3.data);
      ...
      topCards.push(
        <Col span={6} style={{ paddingBottom: "20px" }}>
          <div>
            ...
            >
              {data.name} &nbsp;{" "}
              <Link style={{ color: "white" }} to={`${url}?domain=EdnSys`}>
                <ArrowRightOutlined />
              </Link>
            </StyledCard>
          </div>
  ...
  return (
    ...
    >
      <StyledHeading>EDN SYS</StyledHeading>
      <Row>
        <Col span={24} style={{}}>
          {inventoryCounts ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                {tagIdEdnSysVDate ? (
                  <PieChart>
                    ...
                    <Pie
                      onClick={(e) => handleClick(e)}
                      data={tagIdEdnSysVDate}
                      cx="50%"
                      cy="50%"
                      isAnimationActive={false}
                      label={
                        <CustomPieChartLabel
                          total={getTotal(tagIdEdnSysVDate)}
                        />
                      }
                      outerRadius={100}
                      dataKey="value"
                      ...
                      labelLine={false}
                    >
                      {tagIdEdnSysVDate.map((entry, index) => (
                        <Cell
                          key={`cell-${index}`}

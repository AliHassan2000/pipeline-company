...
const Index = (props) => {
  ...
  const [tagIdIgwNetVDate, setTagIdIgwNetVDate] = useState(null);
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      setLoading(true);
      try {
        const tags = await axios.get(baseUrl + "/tagIdIgwNet");
        setTags(tags.data);
        console.log(tags.data);

        const eos = await axios.get(baseUrl + "/eosExpiryIgwNet");
        setEOSExpiry(eos.data);
        console.log(eos.data);

        const res = await axios.get(baseUrl + "/igwNetInventoryCounts");
        ...
        const res2 = await axios.get(baseUrl + "/igwNetVirtual");
        setVirtualTypes(res2.data);
        // console.log(res2.data);

        const res3 = await axios.get(baseUrl + "/tagIdIgwNetVDate");
        setTagIdIgwNetVDate(res3.data);
      ...
      topCards.push(
        <Col span={6} style={{ paddingBottom: "20px" }}>
          <div>
            ...
            >
              {data.name} &nbsp;{" "}
              <Link style={{ color: "white" }} to={`${url}?domain=IgwNet`}>
                <ArrowRightOutlined />
              </Link>
            </StyledCard>
          </div>
  ...
  return (
    ...
    >
      <StyledHeading>IGW NET</StyledHeading>
      <Row>
        <Col span={24} style={{}}>
          {inventoryCounts ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                {tagIdIgwNetVDate ? (
                  <PieChart>
                    ...
                    <Pie
                      onClick={(e) => handleClick(e)}
                      data={tagIdIgwNetVDate}
                      cx="50%"
                      cy="50%"
                      isAnimationActive={false}
                      label={
                        <CustomPieChartLabel
                          total={getTotal(tagIdIgwNetVDate)}
                        />
                      }
                      outerRadius={100}
                      dataKey="value"
                      ...
                      labelLine={false}
                    >
                      {tagIdIgwNetVDate.map((entry, index) => (
                        <Cell
                          key={`cell-${index}`}

...
const Index = (props) => {
  ...
  const [tagIdIgwSysVDate, setTagIdIgwSysVDate] = useState(null);
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      setLoading(true);
      try {
        const tags = await axios.get(baseUrl + "/tagIdIgwSys");
        setTags(tags.data);
        console.log(tags.data);

        const eos = await axios.get(baseUrl + "/eosExpiryIgwSys");
        setEOSExpiry(eos.data);
        console.log(eos.data);

        const res = await axios.get(baseUrl + "/igwSysInventoryCounts");
        ...
        const res2 = await axios.get(baseUrl + "/igwSysVirtual");
        setVirtualTypes(res2.data);
        // console.log(res2.data);

        const res3 = await axios.get(baseUrl + "/tagIdIgwSysVDate");
        setTagIdIgwSysVDate(res3.data);
      ...
      topCards.push(
        <Col span={6} style={{ paddingBottom: "20px" }}>
          <div>
            ...
            >
              {data.name} &nbsp;{" "}
              <Link style={{ color: "white" }} to={`${url}?domain=IgwSys`}>
                <ArrowRightOutlined />
              </Link>
            </StyledCard>
          </div>
  ...
  return (
    ...
    >
      <StyledHeading>IGW SYS</StyledHeading>
      <Row>
        <Col span={24} style={{}}>
          {inventoryCounts ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                {tagIdIgwSysVDate ? (
                  <PieChart>
                    ...
                    <Pie
                      onClick={(e) => handleClick(e)}
                      data={tagIdIgwSysVDate}
                      cx="50%"
                      cy="50%"
                      isAnimationActive={false}
                      label={
                        <CustomPieChartLabel
                          total={getTotal(tagIdIgwSysVDate)}
                        />
                      }
                      outerRadius={100}
                      dataKey="value"
                      ...
                      labelLine={false}
                    >
                      {tagIdIgwSysVDate.map((entry, index) => (
                        <Cell
                          key={`cell-${index}`}

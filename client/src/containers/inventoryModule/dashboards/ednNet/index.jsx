...
const Index = (props) => {
  ...
  const [tagIdEdnNetVDate, setTagIdEdnNetVDate] = useState(null);
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      setLoading(true);
      try {
        const tags = await axios.get(baseUrl + "/tagIdEdnNet");
        setTags(tags.data);
        console.log(tags.data);

        const eos = await axios.get(baseUrl + "/eosExpiryEdnNet");
        setEOSExpiry(eos.data);
        console.log(eos.data);

        const res = await axios.get(baseUrl + "/ednNetInventoryCounts ");
        ...
        const res2 = await axios.get(baseUrl + "/ednNetVirtual ");
        setVirtualTypes(res2.data);

        const res3 = await axios.get(baseUrl + "/tagIdEdnNetVDate ");
        setTagIdEdnNetVDate(res3.data);
  ...
  return (
    ...
    >
      <StyledHeading>EDN NET</StyledHeading>
      <Row>
        <Col span={24} style={{}}>
          {inventoryCounts ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                {tagIdEdnNetVDate ? (
                  <PieChart>
                    ...
                    <Pie
                      onClick={(e) => handleClick(e)}
                      data={tagIdEdnNetVDate}
                      cx="50%"
                      cy="50%"
                      isAnimationActive={false}
                      label={
                        <CustomPieChartLabel
                          total={getTotal(tagIdEdnNetVDate)}
                        />
                      }
                      outerRadius={100}
                      dataKey="value"
                      ...
                      labelLine={false}
                    >
                      {tagIdEdnNetVDate.map((entry, index) => (
                        <Cell
                          key={`cell-${index}`}

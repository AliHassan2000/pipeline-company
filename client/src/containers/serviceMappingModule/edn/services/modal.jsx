...
const AddDeviceModal = (props) => {
  ...
  let [ip, setIp] = useState(
    device ? getString(device.edn_service_mapping_id) : ""
  );
  ...
  let [serviceVendor, setServiceVendor] = useState(
    device ? getString(device.service_vendor) : ""
  );
  ...
  const postDevice = async (device) => {
    try {
      //console.log(device);
      await axios
        .post(baseUrl + "/addEdnService", device)
        .then(() => {
          const promises = [];
          promises.push(
            axios
              ...
              .then((response) => {
                if (response?.response?.status == 500) {
                  ...
                  openSweetAlert("Record Added Successfully", "success");
                  const promises = [];
                  promises.push(
                    axios
                      .get(baseUrl + "/getAllEdnServices")
        ...
        <Row gutter={30}>
          ...
          <Col span={12}>
            <InputWrapper>
              ...
              &nbsp;&nbsp;
              {device ? (
                <StyledInput value={serverName} readonly />
              ) : (
                <StyledInput
                  value={serverName}
            ...
            <InputWrapper>
              Device B Mac: &nbsp;<span style={{ color: "red" }}>*</span>
              &nbsp;&nbsp;
              <StyledInput
                value={deviceBMac}

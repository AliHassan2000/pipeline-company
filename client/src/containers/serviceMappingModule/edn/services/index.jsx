...
const Index = (props) => {
  ...
  const [syncFromLoading, setSyncFromLoading] = useState("empty");
  const [masterFetchDate, setMasterFetchDate] = useState("");
  const [masterBackgroundColor, setMasterBackgroundColor] = useState("green");
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      ...
      try {
        ...
        let sm = await axios.get(
          baseUrl + "/getEdnServiceMappingSyncFromItStatus"
        );

        if (sm?.data?.fetch_status === "Running") {
          setSyncFromLoading("true");
          setMasterBackgroundColor("red");
        } else if (sm?.data?.fetch_status === "Completed") {
          console.log("here");

          setSyncFromLoading("false");
          setMasterBackgroundColor("green");
        } else {
          setSyncFromLoading("empty");
        }
        setMasterFetchDate(sm?.data?.fetch_date);

        const res = await axios.get(baseUrl + "/getAllEdnServices");
        excelData = res.data;
  ...
  const jsonToExcel = (seedData) => {
    ...
    XLSX.utils.book_append_sheet(wb, binarySeedData, "ednservices");
    XLSX.writeFile(wb, "ednservices.xlsx");
  ...
  const postSeed = async (seed) => {
    setLoading(true);
    await axios
      .post(baseUrl + "/addEdnServices", seed)
      .then((response) => {
        const promises = [];
        promises.push(
          axios
            .get(baseUrl + "/getAllEdnServices")
            .then((response) => {
  ...
  const columns = [
    ...
    // },
    {
      title: "EDN Service Mapping Id",
      dataIndex: "edn_service_mapping_id",
      key: "edn_service_mapping_id",
      // ...getColumnSearchProps("edn_system_id"),
      ...getColumnSearchProps(
        "edn_service_mapping_id",
        "EDN Service Mapping Id",
        setRowCount,
        setDataSource,
        excelData,
        columnFilters
  ...
  const handleDelete = () => {
    if (selectedRowKeys.length > 0) {
      axios
        .post(baseUrl + "/deleteEdnServiceDevice", {
          ips: selectedRowKeys,
        })
        .then(() => {
          const promises = [];
  ...
  const syncFrom = async () => {
    setSyncFromLoading("true");
    setMasterFetchDate(new Date().toLocaleString());
    setMasterBackgroundColor("red");
    await axios
      .get(baseUrl + "/addEdnServiceMapping")
      .then((response) => {
        const promises = [];
        promises.push(
          axios.get(baseUrl + "/getAllEdnServices").then((response) => {
            console.log(response.data);
            setRowCount(response.data.length);
            setDataSource(response.data);
            excelData = response.data;
          ...
          <div style={{ display: "flex" }}>
            ...
            <Spin spinning={syncFromLoading === "true" ? true : false}>
              <StyledButton
                ...
                onClick={syncFrom}
              >
                <RightSquareOutlined /> &nbsp; Sync From It Services
              </StyledButton>
            </Spin>
            &nbsp;
            {syncFromLoading === "empty" ? null : (
              ...
              >
                {syncFromLoading === "true"
                  ? `Syncing Started At: ${masterFetchDate}`
                  : `Syncing Completed At: ${masterFetchDate}`}
              </div>
            )}
          </div>

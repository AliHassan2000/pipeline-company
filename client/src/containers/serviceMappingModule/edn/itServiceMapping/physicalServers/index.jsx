...
const Index = (props) => {
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      ...
      try {
        ...
        const res = await axios.get(baseUrl + "/physicalServers");
        excelData = res.data;
        // console.log("papi");
        // console.log(res);
  ...
  const jsonToExcel = (seedData) => {
    ...
    XLSX.utils.book_append_sheet(wb, binarySeedData, "physicalservers");
    XLSX.writeFile(wb, "physicalservers.xlsx");
  ...
  const postSeed = async (seed) => {
    setLoading(true);
    await axios
      .post(baseUrl + "/physicalServersNew", seed)
      .then((response) => {
        const promises = [];
        promises.push(
          axios
            .get(baseUrl + "/physicalServers")
            .then((response) => {
  ...
  const handleDelete = () => {
    if (selectedRowKeys.length > 0) {
      axios
        .delete(baseUrl + "/physicalServers", {
          data: {
            ips: selectedRowKeys,
          },
        })

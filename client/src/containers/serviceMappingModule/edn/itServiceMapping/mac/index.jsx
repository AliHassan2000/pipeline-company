...
const Index = (props) => {
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      ...
      try {
        ...
        const res = await axios.get(baseUrl + "/macTable");
        excelData = res.data;
        // console.log("papi");
        // console.log(res);
  ...
  const jsonToExcel = (seedData) => {
    ...
    XLSX.utils.book_append_sheet(wb, binarySeedData, "mac");
    XLSX.writeFile(wb, "mac.xlsx");
  ...
  const postSeed = async (seed) => {
    setLoading(true);
    await axios
      .post(baseUrl + "/macTableNew", seed)
      .then((response) => {
        const promises = [];
        promises.push(
          axios
            .get(baseUrl + "/macTable")
            .then((response) => {

...
const Index = (props) => {
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      ...
      try {
        await axios
          .get(baseUrl + "/getAllIgwLldpLegacyDates")
          .then((response) => {
            setDates(response.data);
            console.log(response);
          });
        const status = await axios.get(
          baseUrl + "/getIgwLldpLegacyFetchStatus"
        );
        console.log("popo");
        console.log(status);
        ...
        const res = await axios.get(baseUrl + "/getAllIgwLldpLegacy");
        excelData = res.data;
  ...
  const jsonToExcel = (seedData) => {
    ...
    XLSX.writeFile(wb, "igwLldplegacy.xlsx");
  ...
  const exportSeed = async () => {
    ...
    setExportLoading(false);
    // await axios
    //   .get(baseUrl + "/exportIgwLldpLegacy")
    //   .then((response) => {
    //     openNotification();
  ...
  const fetch = async () => {
    ...
    setBackgroundColor("red");
    await axios
      .get(baseUrl + "/fetchIgwLldpLegacy")
      .then((response) => {
  ...
  const columns = [
    ...
    {
      title: "IGW LLDP",
      dataIndex: "igw_lldp_legacy_id",
      key: "igw_lldp_legacy_id",
  ...
  const handleSelectChange = async (value) => {
    setLoading(true);
    await axios
      .post(baseUrl + "/getAllIgwLldpLegacyByDate", { date: value })
      .then((response) => {
        excelData = response.data;
    ...
    <>
      <Spin tip="Loading..." spinning={loading}>
        ...
        <StyledTable
          ...
          size="small"
          scroll={{ x: 7000, y: height - 350 }}
          onChange={handleChange}
          // rowSelection={rowSelection}
          columns={columns}
          dataSource={dataSource}
          rowKey="igw_lldp_legacy_id"

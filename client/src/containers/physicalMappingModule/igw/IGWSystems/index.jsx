...
const Index = (props) => {
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      ...
      try {
        await axios.get(baseUrl + "/getAllIgwSystemDates").then((response) => {
          setDates(response.data);
          console.log(response);
        });

        const res = await axios.get(baseUrl + "/getAllIgwSystems");
        excelData = res.data;
  ...
  const jsonToExcel = (seedData) => {
    ...
    XLSX.writeFile(wb, "igwsystems.xlsx");
  ...
  const handleDelete = () => {
    if (selectedRowKeys.length > 0) {
      axios
        .post(baseUrl + "/deleteIgwSystemDevice", {
          ips: selectedRowKeys,
        })
        .then(() => {
          const promises = [];
  ...
  const postSeed = async (seed) => {
    setLoading(true);
    await axios
      .post(baseUrl + "/addIgwSystemDevices", seed)
      .then((response) => {
        const promises = [];
        promises.push(
          axios
  ...
  const columns = [
    ...
    {
      title: "IGW System Id",
      dataIndex: "igw_system_id",
      key: "igw_system_id",
      // ...getColumnSearchProps("igw_system_id"),
      ...
      // sorter: (a, b) => a.igw_system_id.length - b.igw_system_id.length,
      // sortOrder: sortedInfo.columnKey === "igw_system_id" && sortedInfo.order,
      ellipsis: true,
    },
    {
      title: "Device A Name",
  ...
  const handleSelectChange = async (value) => {
    setLoading(true);
    await axios
      .post(baseUrl + "/getAllIgwSystemByDate", { date: value })
      .then((response) => {
        excelData = response.data;
        setDataSource(response.data);

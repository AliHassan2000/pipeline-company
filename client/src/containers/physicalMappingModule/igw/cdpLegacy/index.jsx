...
const Index = (props) => {
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      ...
      try {
        await axios
          .get(baseUrl + "/getAllIgwCdpLegacyDates")
          .then((response) => {
            setDates(response.data);
            console.log(response);
          });
        const status = await axios.get(baseUrl + "/getIgwCdpLegacyFetchStatus");
        console.log("popo");
        console.log(status);
        ...
        const res = await axios.get(baseUrl + "/getAllIgwCdpLegacy");
        excelData = res.data;
  ...
  const jsonToExcel = (seedData) => {
    ...
    XLSX.writeFile(wb, "igwcdplegacy.xlsx");
  ...
  const exportSeed = async () => {
    ...
    setExportLoading(false);
    // await axios
    //   .get(baseUrl + "/exportIgwCdpLegacy")
    //   .then((response) => {
    //     openNotification();
  ...
  const columns = [
    ...
    {
      title: "IGW CDP Legacy",
      dataIndex: "igw_cdp_legacy_id",
      key: "igw_cdp_legacy_id",
  ...
  const handleSelectChange = async (value) => {
    setLoading(true);
    await axios
      .post(baseUrl + "/getAllIgwCdpLegacyByDate", { date: value })
      .then((response) => {
        excelData = response.data;

...
const Index = (props) => {
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      ...
      try {
        ...
        const res = await axios.get(baseUrl + "/getAllEdnIpt");
        excelData = res.data;
  ...
  const postSeed = async (seed) => {
    setLoading(true);
    await axios
      .post(baseUrl + "/addEdnIptDevices", seed)
      .then((response) => {
        const promises = [];
        promises.push(
          axios
            .get(baseUrl + "/getAllEdnIpt")
            .then((response) => {
  ...
  const handleDelete = () => {
    if (selectedRowKeys.length > 0) {
      axios
        .post(baseUrl + "/deleteEdnIptDevice", {
          ips: selectedRowKeys,
        })
        .then(() => {
          const promises = [];

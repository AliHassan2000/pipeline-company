...
const Index = (props) => {
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      ...
      try {
        await axios.get(baseUrl + "/getAllEdnMplsDates").then((response) => {
          setDates(response.data);
          console.log(response);
        });

        const res = await axios.get(baseUrl + "/getAllEdnMpls");
        excelData = res.data;
  ...
  const jsonToExcel = (seedData) => {
    ...
    XLSX.writeFile(wb, "ednmpls.xlsx");
  ...
  const postSeed = async (seed) => {
    setLoading(true);
    await axios
      .post(baseUrl + "/addEdnMplsDevices", seed)
      .then((response) => {
        const promises = [];
        promises.push(
          axios
  ...
  const columns = [
    ...
    {
      title: "EDN MPLS Id",
      dataIndex: "edn_mpls_id",
      key: "edn_mpls_id",
      // ...getColumnSearchProps("edn_mpls_id"),
      ...
      // sorter: (a, b) => a.edn_mpls_id.length - b.edn_mpls_id.length,
      // sortOrder: sortedInfo.columnKey === "edn_mpls_id" && sortedInfo.order,
      ellipsis: true,
    },
    {
      title: "Device A Name",
  ...
  const handleDelete = () => {
    if (selectedRowKeys.length > 0) {
      axios
        .post(baseUrl + "/deleteEdnMplsDevice", {
          ips: selectedRowKeys,
        })
        .then(() => {
          const promises = [];
  ...
  const handleSelectChange = async (value) => {
    setLoading(true);
    await axios
      .post(baseUrl + "/getAllEdnMplsByDate", { date: value })
      .then((response) => {
        excelData = response.data;
        setDataSource(response.data);

...
const Index = (props) => {
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      ...
      try {
        await axios.get(baseUrl + "/getAllEdnSystemDates").then((response) => {
          setDates(response.data);
          console.log(response);
        });

        const res = await axios.get(baseUrl + "/getAllEdnSystems");
        excelData = res.data;
        // console.log("papi");
        // console.log(res);
  ...
  const jsonToExcel = (seedData) => {
    ...
    XLSX.writeFile(wb, "ednsystems.xlsx");
  ...
  const postSeed = async (seed) => {
    setLoading(true);
    await axios
      .post(baseUrl + "/addEdnSystemDevices", seed)
      .then((response) => {
        const promises = [];
        promises.push(
          axios
            .get(baseUrl + "/getAllEdnSystems")
            .then((response) => {
  ...
  const handleDelete = () => {
    if (selectedRowKeys.length > 0) {
      axios
        .post(baseUrl + "/deleteEdnSystemDevice", {
          ips: selectedRowKeys,
        })
        .then(() => {
          const promises = [];
  ...
  const handleSelectChange = async (value) => {
    setLoading(true);
    await axios
      .post(baseUrl + "/getAllEdnSystembyDate", { date: value })
      .then((response) => {
        excelData = response.data;
        setDataSource(response.data);

...
const Index = (props) => {
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      ...
      try {
        await axios
          .get(baseUrl + "/getAllEdnSecurityDates")
          .then((response) => {
            setDates(response.data);
            console.log(response);
          });

        const res = await axios.get(baseUrl + "/getAllEdnSecurities");
        excelData = res.data;
  ...
  const jsonToExcel = (seedData) => {
    ...
    XLSX.writeFile(wb, "ednsecurity.xlsx");
  ...
  const postSeed = async (seed) => {
    setLoading(true);
    await axios
      .post(baseUrl + "/addEdnSecurityDevices", seed)
      .then((response) => {
        const promises = [];
        promises.push(
          axios
            .get(baseUrl + "/getAllEdnSecurities")
            .then((response) => {
  ...
  const columns = [
    ...
    // },
    {
      title: "EDN Securuity Id",
      dataIndex: "edn_security_id",
      key: "edn_security_id",
      // ...getColumnSearchProps("edn_security_id"),
      ...
      // sorter: (a, b) => a.edn_security_id.length - b.edn_security_id.length,
      // sortOrder: sortedInfo.columnKey === "edn_security_id" && sortedInfo.order,
      ellipsis: true,
    },
    {
      title: "Device A Name",
  ...
  const handleDelete = () => {
    if (selectedRowKeys.length > 0) {
      axios
        .post(baseUrl + "/deleteEdnSecurityDevice", {
          ips: selectedRowKeys,
        })
        .then(() => {
          const promises = [];
  ...
  const handleSelectChange = async (value) => {
    setLoading(true);
    await axios
      .post(baseUrl + "/getAllEdnSecuritybyDate", { date: value })
      .then((response) => {
        excelData = response.data;
        setDataSource(response.data);

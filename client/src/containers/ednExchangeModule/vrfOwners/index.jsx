...
const Index = (props) => {
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      ...
      try {
        // await axios.get(baseUrl + "/getAllEdnIptDates").then((response) => {
        //   setDates(response.data);
        //   console.log(response);
        // });

        const res = await axios.get(baseUrl + "/getAllVrfOwners");
        excelData = res.data;
  ...
  const jsonToExcel = (seedData) => {
    ...
    XLSX.writeFile(wb, "vrfowners.xlsx");
  ...
  const postSeed = async (seed) => {
    setLoading(true);
    await axios
      .post(baseUrl + "/addVrfOwners", seed)
      .then((response) => {
        const promises = [];
        promises.push(
          axios
  ...
  const handleDelete = () => {
    if (selectedRowKeys.length > 0) {
      axios
        .post(baseUrl + "/deleteVrfOwner", {
          vrf_owners_ids: selectedRowKeys,
        })
        .then(() => {
          const promises = [];
          promises.push(
  ...
  const handleSelectChange = async (value) => {
    setLoading(true);
    await axios
      .post(baseUrl + "/getAllEdnIptByDate", { date: value })
      .then((response) => {
        excelData = response.data;
        setDataSource(response.data);

...
import SpofVRFs from "./spofVRFsTable";
import UnusedVRFs from "./unUsedVRFsTable";
import MissingRoutesVRFs from "./missingRoutesVRFsTable";
import MissingRoutesIntranetVRFs from "./missingRoutesIntranetVRFs";
...
const Index = (props) => {
  ...
  //       devices: { view: false, read_only: false },
  //     },
  //   },
  //   atom: {
  //     view: false,
  //     pages: {
  //       atom: { view: false, read_only: false },
  //       password_group: { view: false, read_only: false },
  //     },
  //   },
  // });
  ...
  let [spofVRFsData, setSpofVRFsData] = useState();
  // let [virtualTypes, setVirtualTypes] = useState();
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      setLoading(true);
      try {
        const res = await axios.get(baseUrl + "/ednExchangeVrfCount");
        setInventoryCounts(res.data);

        const tags = await axios.get(baseUrl + "/getVrfPerRegion");
        setTags(tags.data);

        const res5 = await axios.get(baseUrl + "/getVrfPerSiteId");
  ...
  return (
    ...
    >
      <StyledHeading>EDN Exchange Dashboard</StyledHeading>
      <Row>
        <Col span={24} style={{}}>
          {inventoryCounts ? (
            ...
            >
              ...
              <ResponsiveContainer width="100%" height="100%">
                {tags ? (
                  ...
                  >
                    <Spin tip="Loading Pie Chart..." spinning={true} />
                  </div>
                )}
              </ResponsiveContainer>
            </StyledCard>

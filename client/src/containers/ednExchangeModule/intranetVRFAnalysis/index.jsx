...
const Index = (props) => {
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      ...
      try {
        await axios
          .get(baseUrl + "/getAllIntranetVRFAnalysisDates")
          .then((response) => {
        ...
        const res = await axios.get(baseUrl + "/getAllIntranetVRFAnalysis");
        excelData = res.data;
  ...
  const jsonToExcel = (seedData) => {
    ...
    XLSX.utils.book_append_sheet(wb, binarySeedData, "intranetvrfanalysis");
    XLSX.writeFile(wb, "intranetvrfanalysis.xlsx");
  ...
  const postSeed = async (seed) => {
    setLoading(true);
    await axios
      .post(baseUrl + "/intranetVRFAnalysis", seed)
      .then((response) => {
        const promises = [];
        promises.push(
          axios
            .get(baseUrl + "/intranetVRFAnalysis")
            .then((response) => {
  ...
  const handleSelectChange = async (value) => {
    setLoading(true);
    await axios
      .post(baseUrl + "/getAllIntranetVRFAnalysisByDate", { date: value })
      .then((response) => {
        excelData = response.data;
    ...
    <>
      <Spin tip="Loading..." spinning={loading}>
        ...
        <StyledTable
          ...
          columns={columns}
          dataSource={dataSource}
          rowKey="internal_vrf_analysis_id"

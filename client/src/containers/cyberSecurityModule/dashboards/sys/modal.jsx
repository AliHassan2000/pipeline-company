...
const AddDeviceModal = (props) => {
  ...
  useEffect(() => {
    const serviceCalls = async () => {
      try {
        const res1 = await axios.get(baseUrl + "/getFunctions?domain=SYSTEM");
        const res2 = await axios.post(baseUrl + "/getPnCodes?domain=SYSTEM", {
          functions: [],
        });
  ...
  const handleFunctionsChange = async (e) => {
    ...
    const res = await axios.post(baseUrl + "/getPnCodes?domain=SYSTEM", {
      functions,
    });

...
class JuniperPuller(object):
    ...
    def poll(self, host):
        ...
        while c < login_tries :
            try:
                device = Netmiko(host=host['host'], username=host['user'], password=host['pwd'], device_type='juniper_junos', timeout=600, global_delay_factor=2)
        ...
        if is_login==True:    
            ...
            while c < 3:      #trying 3 times of inventory if gets failed
                ...
                try:
                    inv = device.send_command('show chassis hardware |no-more', use_textfsm=True)  
                    break
                except:
                    c +=1
                    time.sleep(1.5)

...
class IOSPuller(object):
    ...
    def poll(self, host):
        ...
        while c < login_tries :
            try:
                device = Netmiko(host=host['host'], username=host['user'], password=host['pwd'], device_type='cisco_ios', timeout=600, global_delay_factor=2, banner_timeout=300)
                print(f"Success: logged in {host['host']}")
                is_login = True
                break
            except Exception as e:
                c +=1
                print(f"Failed to login {host['host']}")

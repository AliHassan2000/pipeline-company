...
class Parse(object):
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    failed_device.append({"ip_address": ip,"date":  time.strftime("%d-%m-%Y"), "time": time.strftime("%H-%M-%S"), "reason":"GET request failed"})
                    try:
        ...
        try:
            ...
            for x in command_list:
                ...
                if formattedOP !='':
                    ...
                    cmd = cmd.strip()
                    outputs.append({cmd:self.parse_output(formattedOP, x.get('template') , x.get('command'))})
    ...
    def parse_output(self,raw_text_data, template, command):
        ...
        try:
            template = open("app/pullers/Arbor/"+template+".textfsm")
